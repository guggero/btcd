...
func verifyMessageForChallenge(message, challengeScript []byte,
	...
	case strings.HasPrefix(signature, PrefixProofOfFunds):
		...
		toSpendHash := buildToSpendTx(message, challengeScript).TxHash()
		err = validateToSignTx(signaturePacket.UnsignedTx, toSpendHash)
		if err != nil {
			return false, empty, err
		}

		// BIP-322 upgradeable rules: tx version must be 0 or 2.
		err = validateUpgradeableRules(
			signaturePacket.UnsignedTx.Version,
		)
		if err != nil {
			return false, empty, err
		}
...
func VerifyMessagePoF(message, pkScript []byte,
	...
	for idx := range toSign.Inputs {
		...
		// (or SIGHASH_DEFAULT for BIP341 P2TR inputs). The script
		// engine does not enforce a specific sighash flag on its own,
		// so we inspect the (finalized) witness and signature script
		// of each input up front.
		err = validateSigHashFlags(
