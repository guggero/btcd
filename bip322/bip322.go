...
func deduplicateNonWitnessUtxos(packet *psbt.Packet) {
	seen := make(map[chainhash.Hash]struct{}, len(packet.Inputs))
	for idx := range packet.Inputs {
		prevTx := packet.Inputs[idx].NonWitnessUtxo
		if prevTx == nil {
			continue
		}

		// Preserve the first occurrence of each transaction and omit
		// all later copies. The first packet input is treated
		// identically here: it is always the first possible occurrence
		// and therefore remains present as required by BIP-322.
...
// validateSignaturePacket validates that the given packet is structurally sane
// and complete to contain a BIP-322 signature of one of the three accepted
// variants.
func validateSignaturePacket(packet *psbt.Packet) error {
	...
	strippedTxSize := packet.UnsignedTx.SerializeSizeStripped()
	...
	// Any witness on an input must be valid now. This allows us to take a
	// shortcut later for the simple variant and makes sure we don't pass
	// any invalid signatures to the psbt.Extract function in the PoF case.
	for idx, pIn := range packet.Inputs {
		// Legacy inputs don't have a witness, we can skip them.
		if len(pIn.FinalScriptWitness) == 0 {
			continue
		}

		_, err := ParseTxWitness(pIn.FinalScriptWitness)
		if err != nil {
			return fmt.Errorf("error parsing input %d witness: %w",
				idx, err)
		}
	}
...
func SerializeSignature(packet *psbt.Packet) (string, error) {
	// Check that the packet is complete/finalized.
	if err := validateSignaturePacket(packet); err != nil {
		return "", err
	}
	...
	default:
		// We've parsed the witness before, so we know it's valid.
		content := b64Encode(
			finalizedPacket.Inputs[0].FinalScriptWitness,
		)
...
func ParseTxWitness(rawWitness []byte) (wire.TxWitness, error) {
	...
	allocCount := min(witCount, txscript.MaxStackSize)
...
// collectUtxos collects all UTXOs referenced by the packet's inputs. It
// implements the special BIP-0322 optimization where multiple non-witness
// inputs that spend from the same transaction can re-use the same
// NonWitnessUtxo field.
func collectUtxos(packet *psbt.Packet) (map[wire.OutPoint]*wire.TxOut, error) {
	// Record validated non-witness transactions as inputs are processed in
	// order. This permits constant-time reuse lookups while ensuring an
	// input can only reuse a transaction supplied by an earlier additional
	// input.
	knownNonWitnessUtxos := make(
		map[chainhash.Hash]*wire.MsgTx, len(packet.Inputs),
	)
	...
	findUtxo := func(idx int) (*wire.TxOut, error) {
		...
		case in.NonWitnessUtxo != nil:
			...
			prevHash := prevTx.TxHash()
			if prevHash != txIn.PreviousOutPoint.Hash {
			...
			// When both representations are present, they must
			// describe the same output. Otherwise, verification and
			// callers that inspect the compact witness UTXO could
			// disagree about the authenticated script or amount.
			if in.WitnessUtxo != nil &&
				!psbt.TxOutsEqual(in.WitnessUtxo, prevOut) {

				return nil, fmt.Errorf("witness utxo does "+
					"not match non witness utxo for input "+
					"index %d", idx)
			}

			// Input zero spends the synthetic to_spend transaction
			// and cannot provide reusable funding information.
			// Every later validated transaction becomes available
			// to subsequent inputs.
			if idx > 0 {
				knownNonWitnessUtxos[prevHash] = prevTx
			}
		...
		default:
			...
			// Only transactions recorded by earlier additional
			// inputs are eligible for reuse, as required by
			// BIP-322.
			prevHash := txIn.PreviousOutPoint.Hash
			prevTx, ok := knownNonWitnessUtxos[prevHash]
			if !ok {
				return nil, fmt.Errorf("invalid signature "+
					"packet: UTXO not found for input "+
					"index %d", idx)
			}
	...
	utxos := make(map[wire.OutPoint]*wire.TxOut, len(packet.Inputs))
	for idx := range packet.Inputs {
		txIn := packet.UnsignedTx.TxIn[idx]
		...
		if exists {
			return nil, fmt.Errorf("duplicate inputs present")
		}
		...
		if txIn.PreviousOutPoint.Hash == (chainhash.Hash{}) {
			return nil, fmt.Errorf("zero hash present in input")
		}

		var err error
		utxos[txIn.PreviousOutPoint], err = findUtxo(idx)
		if err != nil {
			return nil, fmt.Errorf("error finding UTXO: %w", err)
		}
	}
...
func VerifyMessagePoF(message, pkScript []byte,
	...
	// Do some basic validation on the signature packet. We use the
	// validateSignaturePacket function that checks the inputs and UTXO for
	// the first input at least.
	err := validateSignaturePacket(sigPacket)
	if err != nil {
	...
	// Validate and collect all UTXOs now.
	utxos, err := collectUtxos(toSign)
	if err != nil {
		return false, empty, err
	}
	...
	// Now that we've added all the witness data, we need to check the
	// weight again, to check it's still consensus valid. The calculation
	// is a copy of blockchain.GetTransactionWeight, which is just
	// (baseSize * 3) + totalSize.
	baseSize := finalTx.SerializeSizeStripped()
	totalSize := finalTx.SerializeSize()
	if ((baseSize * 3) + totalSize) > maxWitnessItems {
		return false, empty, fmt.Errorf("serialized transaction "+
			"weight exceeds maximum of %d", maxWitnessItems)
	}
	...
	for idx := range toSign.Inputs {
		...
		//
		// BIP-322 also requires MINIMALIF in every script context. The
		// standard flag enforces it only for SegWit v0, while taproot
		// makes it mandatory independently, so the opt-in all-contexts
		// flag closes the remaining legacy-script gap.
		vm, err := txscript.NewEngine(
			utxo.PkScript, finalTx, idx,
			txscript.StandardVerifyFlags|
				txscript.ScriptVerifyRestrictSigHash|
				txscript.ScriptVerifyMinimalIfAll,
