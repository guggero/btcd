...
func TestFullSignaturesVectors(t *testing.T) {
	validateFullVector := func(t *testing.T, tc fullSignatureVector) {
		...
		// And make sure an incorrect message produces a negative
		// result. The error can be either a script verification
		// failure ("invalid signature") or a BIP-322 basic-validation
		// failure ("invalid to_sign transaction") if the wrong message
		// is detected at the to_sign first input prev outpoint check.
		valid, _, err = VerifyMessage(
			"incorrect message", addr, tc.Bip322Signatures[0],
		)
		require.Error(t, err)
		require.True(
			t, errors.Is(err, ErrInvalidSignature) ||
				errors.Is(err, ErrInvalidToSign),
			"expected ErrInvalidSignature or ErrInvalidToSign, "+
				"got %v", err,
		)
		require.False(t, valid, "signature of fake msg must be invalid")
	}
