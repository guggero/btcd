...
const (
	errorResultInvalid      = "invalid"
	errorResultInconclusive = "inconclusive"
)
...
type testVectors struct {
	...
	Simple       []simpleSignatureVector `json:"simple,omitempty"`
	Full         []fullSignatureVector   `json:"full,omitempty"`
	ProofOfFunds []pofSignatureVector    `json:"proof_of_funds,omitempty"`
	Valid        []validVector           `json:"valid,omitempty"`
	Error        []errorVector           `json:"error,omitempty"`
}
...
// "Proof of Funds" variant test case.
type pofSignatureVector struct {
	fullSignatureVector

	// AdditionalInputs is a two-dimensional slice that defines inputs we
	// want to add to a Proof of Funds transaction, grouped by previous
	// transaction.
	AdditionalInputs [][]additionalInput `json:"additional_inputs"`
}

// errorVector contains a test case that must fail verification. ExpectedResult
// preserves BIP-322's distinction between a definitively invalid proof and one
// that might become valid under upgraded rules.
...
// validVector contains a static signature that must verify successfully. Unlike
// the simple/full/proof_of_funds vectors, these are not re-signed from private
// keys; they hold a fixed signature that VerifyMessage must accept. This is used
// for spends that can't be expressed as a standard, re-signable input type
// (e.g. custom tapscript or witness scripts).
...
// parseWIFs parses a list of WIFs into a slice of *address.WIF.
...
// formatWIFs formats a slice of *address.WIF into a slice of strings.
...
func TestSimpleSignaturesVectors(t *testing.T) {
	validateSimpleVector := func(t *testing.T, tc simpleSignatureVector) {
		...
		witnessBytes, err := b64StrictDecode(signature[len(PrefixSimple):])
		require.NoError(t, err)
...
func TestFullSignaturesVectors(t *testing.T) {
	validateFullVector := func(t *testing.T, tc fullSignatureVector) {
		...
		toSignTx, err := BuildToSignPacketFull(
		...
		// And make sure an incorrect message produces a negative
		// result. The error should be that the wrong message is
		// detected at the to_sign first input prev outpoint check.
		valid, _, err = VerifyMessage(
...
// testErrorVectors verifies all error vectors and enforces BIP-322's
// invalid-versus-inconclusive distinction.
func testErrorVectors(t *testing.T, vectors *testVectors) {
	t.Helper()

	for _, testCase := range vectors.Error {
		t.Run(testCase.Description, func(t *testing.T) {
			addr, err := address.DecodeAddress(
				testCase.Address, netParams,
			)
			require.NoError(t, err)

			valid, _, err := VerifyMessage(
				testCase.Message, addr, testCase.Signature,
			)
			...
			case errorResultInvalid:
				require.NotErrorIs(t, err, ErrInconclusive)

			case errorResultInconclusive:
				require.ErrorIs(t, err, ErrInconclusive)

			default:
				require.Failf(
					t, "unknown expected result",
					"result=%q", testCase.ExpectedResult,
				)
			}
		})
	}
...
func TestErrorVectors(t *testing.T) {
	for _, fileName := range []string{
		...
		negativeVectorFileName,
	} {

		t.Run(fileName, func(t *testing.T) {
			testErrorVectors(t, loadTestVectors(t, fileName))
		})
	}
}

// TestValidVectors tests that the static "valid" signature vectors (spends that
// cannot be expressed as a standard, re-signable input type) verify
// successfully.
func TestValidVectors(t *testing.T) {
	testValidVectors := func(t *testing.T, vectors *testVectors) {
...
func (i testInputType) output(t *testing.T,
	...
	switch i {
	case plainP2PKH:
		h := address.Hash160(privKey.PubKey().SerializeCompressed())
		addr, err = address.NewAddressPubKeyHash(h, netParams)
		require.NoError(t, err)

	case plainP2WPKH:
		h := address.Hash160(privKey.PubKey().SerializeCompressed())
		addr, err = address.NewAddressWitnessPubKeyHash(h, netParams)
		require.NoError(t, err)
	...
	case nestedP2WPKH:
		witnessProgram, err := payToWitnessPubKeyHashScript(privKey)
		require.NoError(t, err)
	...
	case timeLockP2WSH:
		...
		addr, err = address.NewAddressWitnessScriptHash(h[:], netParams)
		require.NoError(t, err)
	...
	case nestedMultisig2of2P2WSH:
		...
		h := sha256.Sum256(witnessScript)
		wshAddr, err := address.NewAddressWitnessScriptHash(
			h[:], netParams,
		)
		require.NoError(t, err)
		...
		addr, err = address.NewAddressScriptHash(wshScript, netParams)
		require.NoError(t, err)
		sigScript = wshScript
	...
	case legacyMultisig2of2P2SH:
		...
		addr, err = address.NewAddressScriptHash(sigScript, netParams)
		require.NoError(t, err)
	...
	return txOut, addr.String(), signingKeys, witnessScript, sigScript
}
...
// flow end-to-end for the "Proof of Funds" variant.
...
func runTestCaseSimple(t *testing.T,
	...
	parsedAddr, err := address.DecodeAddress(addr, netParams)
	require.NoError(t, err)
...
func runTestCasePoF(t *testing.T, challengeInputType testInputType,
	...
	toSign, err := BuildToSignPacketFull(
		message, txOut.PkScript, 2, 123, 456,
	)
	require.NoError(t, err)
	...
	// We also strip the generic signed message field, as that's not needed
	// inside the "signature" part, since it will always be transported to
	// the verifier as part of the "message, address, signature" triplet.
	toSign.GenericSignedMessage = nil
