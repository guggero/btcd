...
func argCheck(node *AST) (*AST, error) {
	...
	case f_older, f_after:
		...
		_n := node.args[0]
		if err := checkValueArg(_n); err != nil {
			return nil, err
		}
	...
	case f_thresh, f_multi, f_multi_a, f_sortedmulti_a:
		...
		_k := node.args[0]
		if err := checkValueArg(_k); err != nil {
			return nil, err
		}
...
func malleabilityCheck(node *AST) (*AST, error) {
	...
	case f_thresh:
		...
		// BIP379's rule is "e=all are s". A threshold whose children
		// are all signed but not all expressive is "e" here even
		// though a third party can vary the dissatisfaction of such a
		// child; the m property computed above does require every
		// child to be expressive, and malleability is conjunctive, so
		// every expression containing such a threshold is malleable
		// anyway.
		node.props.e = true
		for _, arg := range node.args[1:] {
