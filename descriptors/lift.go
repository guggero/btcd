...
const (
	// missingField is rendered by String in place of an optional field that
	// a policy of the corresponding type left unset. SemanticPolicy is an
	// exported type with JSON tags, so it can reach String hand-built or
	// unmarshaled from a document that omits the field, which must not
	// panic.
...
//
// A policy that Lift did not produce may leave the optional field its type
// calls for unset, in which case that field renders as missingField rather than
// panicking.
func (p *SemanticPolicy) String() string {
	...
	case SemanticPolicyTypeKey:
		return fmt.Sprintf("pk(%s)", optString(p.Key))

	case SemanticPolicyTypeAfter:
		return fmt.Sprintf("after(%s)", optNum(p.LockTime))

	case SemanticPolicyTypeOlder:
		return fmt.Sprintf("older(%s)", optNum(p.LockTime))

	case SemanticPolicyTypeSha256, SemanticPolicyTypeHash256,
		SemanticPolicyTypeRipemd160, SemanticPolicyTypeHash160:

		return fmt.Sprintf("%s(%s)", p.Type, optString(p.Hash))
	...
	case SemanticPolicyTypeThresh:
		...
		// Without a threshold there is no telling which of the three
		// forms below the policy meant, so it renders as the general
		// one with the value missing.
		if p.Threshold == nil {
			return fmt.Sprintf("thresh(%s,%s)", missingField,
				strings.Join(subs, ","))
		}
...
// optString renders an optional string field of a policy.
...
// optNum renders an optional numeric field of a policy.
func optNum(v *uint32) string {
	if v == nil {
		return missingField
	}
