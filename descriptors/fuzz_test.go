...
const (
	...
	// stay practical while leaving room for varied descriptor and script
	// shapes. Nesting limits alone do not bound the number of sibling
	// nodes.
	maxFuzzInput = 4096

	// gPointX is the x coordinate of the secp256k1 generator, used to build
	// seed descriptors with valid raw keys (which, unlike the xpub seeds,
	// survive mutation without breaking base58 decoding).
	gPointX = "79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16" +
		"f81798"
)

var (
	// hardcodedDescriptorSeeds keep the seed set representative even if the
	// corpus file is unavailable, and add raw-key forms that mutate more
	// gracefully than the long xpub corpus entries.
	hardcodedDescriptorSeeds = []string{
		"pk(" + basicTestXpub + ")",
		"pkh(" + basicTestXpub + "/*)",
...
func exerciseDescriptor(t *testing.T, d *Descriptor) {
	...
	require.GreaterOrEqualf(
		t, multipath, 1, "multipath length must be at least 1 for %q",
		s,
	)
	...
	for mp := range uint32(min(multipath, 4)) {
